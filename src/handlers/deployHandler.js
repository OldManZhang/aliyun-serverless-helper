...
async function handleDeploy(resourceName, options) {
  ...
  if (!resourceName) {
    resourceName = await selectResource(resourcesList, 'Select a resource to deploy:');
  ...
  const command = `s ${templateOption} ${resourceName} deploy`.trim();
